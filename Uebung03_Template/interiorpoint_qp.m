function sol = interiorpoint_qp(G, c, A, b, x0, opt)
%INTERIORPOINT_QP Interior point algorithm for quadratic programming
% min  0.5*x'*Q*x + c'*x
% s.t. A*x = b
%      x >= 0
%
...
% ?bergabeargumente:
%
% G,c,A,b   Matrizen und Vektoren welche das Optimierungsproblem definieren
% x0        Anfangslösung
% opt       Struktur mit einstellbaren Optionen
    ...
    if nargin < 5
        fprintf('The arguments G, c, A, b, x0 are mandatory\n');
        return
    end
    if nargin < 6
        opt = [];
    ...
    for k = 1 : opt.MaxIter
        ...
        F = [G *sol.x + c + A'*sol.lambda - sol.mu; A*sol.x - b; X*M*e - sol.tau*e];
        ...
        sol.f = 0.5 * sol.x' * G * sol.x + c' * sol.x;
